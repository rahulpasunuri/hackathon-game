...
var DRAG = 400;
var MAXSPEED = 400;
var loseLevelScore = 5;
var winLevelScore = 15;
...
function preload() {
    ...
    game.load.image('starfield', 'assets/village.png');
    game.load.image('ship', 'assets/player.png');
    game.load.image('bullet', 'assets/heart.png');
    
    
    game.load.image('penalty-image', 'assets/smileys/sad.svg');
    
    
    game.load.image('bonus-image', 'assets/smileys/bigsmile.svg');
    game.load.image('smile', 'people/Smiling_Person3.png');
    game.load.image('enemy-green', 'people/Sad_Person3.png');
    game.load.image('enemy-blue', 'people/Crying_Person3.png');
...
function create() {
   ...
   game.time.events.add(1000, launchGreenEnemy); 
    ...
    liPenalty = game.add.group();
    liPenalty.enableBody = true;
    liPenalty.physicsBodyType = Phaser.Physics.ARCADE;
    liPenalty.createMultiple(5, 'penalty-image');
    liPenalty.setAll('anchor.x', 0.5);
    liPenalty.setAll('anchor.y', 0.5);
    liPenalty.setAll('scale.x', 0.5);
    liPenalty.setAll('scale.y', 0.5);
    liPenalty.setAll('angle', 360);
    liPenalty.forEach(function(penalty){
        //addEnemyEmitterTrail(bonus);
        penalty.body.setSize(penalty.width * 3 / 4, penalty.height * 3 / 4);
        penalty.damageAmount = 20;
        penalty.events.onKilled.add(function(){
            //bonus.trail.kill();
        });
    });
    
    game.time.events.add(200, launchPenalty);
   ...
   game.time.events.add(1000, launchBlueEnemy); 
    ...
    scoreText.render = function () {
        ...
        {
         $("#layerFill" + i).parent().addClass("ins");
        }
        for( var i=1; i<=progress; i++)
        {
         $("#layerFill" + i).parent().addClass("ins");
        }
        for( var j=progress+1; j<=15; j++)
        {
            $("#layerFill" + j).parent().removeClass("ins");
        }
         if( missed >= loseLevelScore)
         {
             if( progress > 0)
                $(".lost-scored").html( progress);
             else
                $(".lost-scored-msg").html("You can always check out the instructions on the right and start over!")
...
function receivePenalty(player, penalty)
{
    penalty.kill();
    if(score<100)
    {
...
function update() {
    ...
    */
    ...
    game.physics.arcade.overlap(player, liPenalty, receivePenalty, null, this);
...
function launchPenalty()
{
    ...
    var bonus = liPenalty.getFirstExists(false);
    if (bonus) {
    ...
    BonusLaunchTimer = game.time.events.add(game.rnd.integerInRange(BonusSpacing, BonusSpacing + 1000), launchPenalty);   
...
function launchBonus()
{
    ...
    //TEST
    //  Send another enemy soon
    BonusLaunchTimer = game.time.events.add(game.rnd.integerInRange(BonusSpacing, BonusSpacing + 3000), launchBonus);   
